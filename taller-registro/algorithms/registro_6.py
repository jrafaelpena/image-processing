...
def bspline_registration(fixed_image, moving_image, initial_transform, number_of_samples: int, 
    ...
    # Optimized optimizer settings for single level
    optimizer = itk.RegularStepGradientDescentOptimizerv4.New()
    
    optimizer.SetLearningRate(1.0)  # Increased learning rate for faster convergence
    optimizer.SetMinimumStepLength(0.01)  # Increased minimum step
    optimizer.SetNumberOfIterations(200)  # Reduced iterations
    optimizer.SetRelaxationFactor(0.5)  # More aggressive relaxation
    optimizer.SetGradientMagnitudeTolerance(1e-6)  # Less strict tolerance
    
    print(f"  Optimizer: RegularStepGradientDescent (Single-level optimized)")
    
    metric = itk.MattesMutualInformationImageToImageMetricv4[image_type, image_type].New()
    metric.SetNumberOfHistogramBins(32)  # Reduced bins for speed
    ...
    # Increased sampling for better accuracy with single level
    sampling_percentage = min(0.1, max(0.02, number_of_samples / total_voxels))
    registration.SetMetricSamplingPercentage(sampling_percentage)
    registration.MetricSamplingReinitializeSeed(76926294)
    
    print(f"  Using sampling percentage: {sampling_percentage:.4f}")
    
    # SINGLE LEVEL ONLY - This is the key change!
    registration.SetNumberOfLevels(1)
    registration.SetShrinkFactorsPerLevel([1])  # No shrinking
    registration.SetSmoothingSigmasPerLevel([0])  # No smoothing
    
    print(f"  Using SINGLE LEVEL registration for speed")
    ...
    def iteration_update():
        try:
            ...
            if iteration % 5 == 0 or iteration < 10:  # More frequent updates
    ...
    print(f"  Starting single-level B-spline registration...")
    ...
    final_bspline = final_composite_transform.GetNthTransform(1)
...
def resample_pet(pet_moving, pet_reference, ct_transform, output_file: str = None):
    ...
    resampler.SetInterpolator(itk.LinearInterpolateImageFunction.New(Input=pet_moving))
...
def main():
    ...
    step_4_bspline = inputs_path / "step_4_bspline.nrrd"
    ...
    bspline_transform_file = inputs_path / "bspline_transform.txt"
    ...
    # REDUCED GRID SIZE for much faster execution
    ...
    if bspline_transform is None:
        print(f"No existing B-spline transform found. Running SINGLE-LEVEL B-spline registration with grid size {bspline_grid_size}...")
        start_time = time.time()
